...
public class Solution {
    private static final String WIRE_PATH_1 =
    ...
    private static final String WIRE_PATH_2 =
    ...
    private static List<String> getWirePathSegmentList(String wirePath) {
        return Arrays.asList(wirePath.split("\\s*,\\s*"));
    }

    private static Point addPointToPathSet(Set<Point> pointOnPath, Point origin,
                                           String newPathSegment) {
        int segmentLength = Integer.parseInt(newPathSegment.substring(1));
        switch (newPathSegment.charAt(0)) {
            case 'R':
                for (int i = origin.y + 1; i < origin.y + segmentLength; i++) {
                    pointOnPath.add(new Point(origin.x, i));
                }
                Point newHead = new Point(origin.x, origin.y + segmentLength);
                pointOnPath.add(newHead);
                return newHead;
            case 'L':
                for (int i = origin.y - 1; i > origin.y - segmentLength; i--) {
                    pointOnPath.add(new Point(origin.x, i));
                }
                newHead = new Point(origin.x, origin.y - segmentLength);
                pointOnPath.add(newHead);
                return newHead;
            case 'U':
                for (int i = origin.x + 1; i < origin.x + segmentLength; i++) {
                    pointOnPath.add(new Point(i, origin.y));
                }
                newHead = new Point(origin.x + segmentLength, origin.y);
                pointOnPath.add(newHead);
                return newHead;
            case 'D':
                for (int i = origin.x - 1; i > origin.x - segmentLength; i--) {
    ...
    private static Set<Point> getAllPointsOnPathSet(List<String> pathSegments) {
        Set<Point> pointsOnPathSet = new HashSet<>();
        Point origin = new Point(0, 0);
        for (String segment : pathSegments) {
            origin = addPointToPathSet(pointsOnPathSet, origin, segment);
        }
        return pointsOnPathSet;
    }

    private static Map.Entry<Point, Integer> addPointAndRankToPathMap(
            Map<Point, Integer> pointToRankMap, Point origin, int rank, String newPathSegment) {
        int segmentLength = Integer.parseInt(newPathSegment.substring(1));
        switch (newPathSegment.charAt(0)) {
            case 'R':
                for (int i = origin.y + 1; i < origin.y + segmentLength; i++) {
                    Point newPoint = new Point(origin.x, i);
                    rank++;
                    if (!pointToRankMap.containsKey(newPoint)) {
                        pointToRankMap.put(newPoint, rank);
                    }
                }
                Point newHead = new Point(origin.x, origin.y + segmentLength);
                rank++;
                if (!pointToRankMap.containsKey(newHead)) {
                    pointToRankMap.put(newHead, rank);
                }
                return Map.entry(newHead, rank);
            case 'L':
                for (int i = origin.y - 1; i > origin.y - segmentLength; i--) {
                    Point newPoint = new Point(origin.x, i);
                    rank++;
            ...
            case 'U':
                for (int i = origin.x + 1; i < origin.x + segmentLength; i++) {
                    Point newPoint = new Point(i, origin.y);
                    rank++;
                    if (!pointToRankMap.containsKey(newPoint)) {
                        pointToRankMap.put(newPoint, rank);
                    }
    ...
    private static Map<Point, Integer> getAllPointOnMapToRankList(List<String> pathSegments) {
        HashMap<Point, Integer> pointsOnPathToRankMap = new HashMap<>();
        Point origin = new Point(0, 0);
        int rank = 0;
        for (String segment : pathSegments) {
            Map.Entry<Point, Integer> entry =
                    addPointAndRankToPathMap(pointsOnPathToRankMap, origin, rank, segment);
            origin = entry.getKey();
            rank = entry.getValue();
        }
        return pointsOnPathToRankMap;
    }
    ...
    private static int getMinimumManhattanDistance() {
        Set<Point> wirePathPointSet1 = getAllPointsOnPathSet(getWirePathSegmentList(WIRE_PATH_1));
        Set<Point> wirePathPointSet2 = getAllPointsOnPathSet(getWirePathSegmentList(WIRE_PATH_2));
        wirePathPointSet1.retainAll(wirePathPointSet2);
        wirePathPointSet1.remove(new Point(0, 0));

        int minManhattanDistance = Integer.MAX_VALUE;
        for (Point point : wirePathPointSet1) {
            int currentManhattanDistance = getManhattanDistance(new Point(0, 0), point);
            if (minManhattanDistance > currentManhattanDistance) {
    ...
    private static int getShortestPathLengthToAnIntersection() {
        Map<Point, Integer> wirePathPointToRankMap1 =
                getAllPointOnMapToRankList(getWirePathSegmentList(WIRE_PATH_1));
        Map<Point, Integer> wirePathPointToRankMap2 =
                getAllPointOnMapToRankList(getWirePathSegmentList(WIRE_PATH_2));
        ...
        for (Point point : wirePathPointSet1) {
            int pathLengthSum =
                    wirePathPointToRankMap1.get(point) + wirePathPointToRankMap2.get(point);
            if (shortedPathLength > pathLengthSum) {
                shortedPathLength = pathLengthSum;
            }
        }
        return shortedPathLength;
    }

    public static void main(String[] args) {
        System.out.println(getMinimumManhattanDistance());
        System.out.println(getShortestPathLengthToAnIntersection());
    }
}